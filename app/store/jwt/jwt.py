...
class JWTAccessor(BaseAccessor):
    ...
    def set_refresh_token_cookie(self, refresh_token: str, response: 'Response', ):
        """Adding a token to cookies."""
        self.access_security.set_refresh_cookie(response, refresh_token,
                                                timedelta(self.settings.refresh_expires_delta))

    def unset_refresh_token_cookie(self, response: 'Response', ):
